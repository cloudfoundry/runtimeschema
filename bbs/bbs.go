...
type ExecutorBBS interface {
	MaintainExecutorPresence(heartbeatIntervalInSeconds uint64, executorID string) (*Presence, chan error, error)
...
type FileServerBBS interface {
	MaintainFileServerPresence(heartbeatIntervalInSeconds uint64, fileServerURL string, fileServerId string) (*Presence, chan error, error)
}
