	...
	Describe("ClaimTask", func() {
		Context("when claiming a pending Task", func() {
			...
			Context("when the etcdClient is out of commission", func() {
				itRetriesUntilStoreComesBack(func() error {
					return bbs.ClaimTask(task.TaskGuid, "executor-ID")
				})
			})
		})
		...
		Context("when claiming a Task that is not in the pending state", func() {
			...
			It("returns an error", func() {
				...
				err = bbs.ClaimTask(task.TaskGuid, "some-other-executor-ID")
				Ω(err).Should(HaveOccurred())
			})
		})
	})
	...
	Describe("FailedToResolveTask", func() {
		...
		Context("when the task is resolving", func() {
			...
			It("should return the task to the completed state", func() {
				err = bbs.FailedToResolveTask(task.TaskGuid)
				Ω(err).ShouldNot(HaveOccurred())
		...
		Describe("preserving success or failure status of task and reason", func() {
			...
			Context("when the task completed successfully", func() {
				BeforeEach(func() {
					err = bbs.CompleteTask(task.TaskGuid, false, "", "a result")
					Ω(err).ShouldNot(HaveOccurred())
